...
public class TargetScanner : MonoBehaviour
{
    ...
    {
        if (PlayerController.Instance == null || PlayerController.Instance.isRespawning) return null;
        ...
        if (useHeightDifference && Mathf.Abs(toPlayer.y + heightOffset) > maxHeightDifference) return null;
        ...
        {
            if (Vector3.Dot(toPlayerFlat.normalized, detector.forward) > Mathf.Cos(detectionAngle * 0.5f * Mathf.Deg2Rad))
            {
                bool canSee = false;
